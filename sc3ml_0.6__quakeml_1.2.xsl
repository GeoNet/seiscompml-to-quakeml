 ...
 * exist:
 *
 *  - IDs : SC3 does not enforce any particular ID restriction. An ID in SC3
 *    has no semantic, it simply must be unique. Hence QuakeML uses ID
 *    restrictions, a convertion of a SC3 to a QuakeML ID must be performed:
 *    'sc3id' -> 'smi:scs/0.6/sc3id'. If no SC3 ID is available but QuakeML
 *    enforces one, a ID is generated by concatenating the constant 'NA-' with
 *    the result of the generate-id() build-in function.
 *  - Position of 'magnitude' and 'stationMagnitude' nodes : Whereas SC3 places
 *    these nodes under the 'origin' element, QuakeML expects them on the same
 *    level as the 'origin' element. When moving these nodes the publicID of the
 *    parent 'origin' is copied to the 'originID' child node of the magnitude
 *    nodes (except an originID was specified in the SC3 sources nodes).
 *
...
<xsl:stylesheet version="1.0"
    ...
    <xsl:variable name="ID_PREFIX" select="'smi:scs/0.6/'"/>
    ...
    <xsl:template match="scs:arrival//scs:timeUsed|scs:horizontalSlownessUsed|scs:backazimuthUsed"/>
    <xsl:template match="scs:origin//scs:stationMagnitude|scs:magnitude"/>
    <xsl:template match="scs:focalMechanism//scs:evaluationMode|scs:evaluationStatus"/>
    <xsl:template match="scs:momentTensor//scs:method|scs:stationMomentTensorContribution|scs:status|scs:cmtName|scs:cmtVersion"/>
    ...
    <xsl:template match="*" mode="originMagnitude">
        ...
        <xsl:element name="{local-name()}">
            ...
            <!-- comment is mandatory in qml -->
            <xsl:if test="not(scs:comment)">
                <comment/>
            </xsl:if>
...
-->
    ...
    <xsl:template name="convertID">
        <xsl:param name="id"/>
        <xsl:value-of select="concat($ID_PREFIX, translate($id, ' ', ''))"/>
    </xsl:template>
